...
interface SimulationSectionProps {
  ...
  updateQuestionAnswer: (
    // Function to update a specific question's answer
    sectionNum: 1 | 2,
    chapterIdx: number,
    questionIdx: number,
...
export default function SimulationSection({
  ...
  const handleAnswerSelect = async (option: string | null) => {
    ...
    const questionIdxInChapter =
      getQuestionIndexInChapter(currentQuestionIndex);
  ...
  const handleSectionComplete = () => {
    setCurrentQuestionIndex(0); // מאתחל לאינדקס ההתחלתי
    onSectionComplete(); // מפעיל את הסיום
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row items-start justify-between gap-4 mb-6">
        ...
        >
          {sectionNum === 1 ? "Complete Section 1" : "Complete Simulation"}
        </button>
      </div>
      ...
      {chapters[currentChapterIndex]?.type === "reading" &&
        currentExercise &&
        currentChapter.passage && (
          <ReadingExerciseCard
            currentExercise={currentExercise}
